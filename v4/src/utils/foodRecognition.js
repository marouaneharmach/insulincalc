// V5.6 — Groq + Llama 4 Scout Vision Food Recognition
// TF-IDF word frequency weighting for smarter food matching
...
const KEYWORD_MAP = {
  ...
  'big mac': ['big mac', 'burger', 'cheeseburger'],
  'whopper': ['whopper', 'burger'],
  'double cheese': ['double cheese', 'burger', 'cheeseburger'],
  'royal cheese': ['royal cheese', 'burger', 'cheeseburger'],
  'mcchicken': ['mcchicken', 'burger', 'poulet'],
  'double steakhouse': ['double steak', 'burger', 'whopper'],
  'chicken burger': ['mcchicken', 'burger', 'poulet'],
  'fish burger': ['filet o fish', 'burger'],
  'filet o fish': ['filet o fish', 'burger'],
  'steak': ['steak', 'steak hache', 'bifteck', 'entrecote', 'cote de boeuf'],
  'frites': ['frites'],
  'pizza': ['pizza'],
...
/**
 * Build word frequency map across all food names (TF-IDF style)
 * Words appearing in many foods are less discriminating
 */
let _wordFreq = null;
function getWordFrequency(foods) {
  if (_wordFreq) return _wordFreq;
  _wordFreq = {};
  for (const f of foods) {
    const words = new Set(normalize(f.name).split(/[\s,()/-]+/).filter(w => w.length >= 4));
    for (const w of words) _wordFreq[w] = (_wordFreq[w] || 0) + 1;
  }
  return _wordFreq;
}
...
export function mapToLocalFoods(results, allFoods) {
  ...
  const wordFreq = getWordFrequency(foods);
  ...
  return results.map(result => {
    ...
    for (const f of foods) {
      ...
      // 3. Word match with TF-IDF weighting — common words score less
      if (score === 0) {
        const frWords = frName.split(/[\s,()/-]+/).filter(w => w.length >= 4);
        const enWords = enName.split(/[\s,()/-]+/).filter(w => w.length >= 4);
        const fnWords = fn.split(/[\s,()/-]+/).filter(w => w.length >= 4);

        for (const word of [...frWords, ...enWords]) {
          for (const fw of fnWords) {
            if (fw.includes(word) || word.includes(fw)) {
              // Penalize words that appear in many food names
              const freq = Math.max(wordFreq[fw] || 1, wordFreq[word] || 1);
              const s = (100 + Math.min(word.length, fw.length)) / freq;
    ...
    const mapped = best != null && bestScore >= 50;
