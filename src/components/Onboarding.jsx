...
export default function Onboarding({ onComplete, t, locale, isRTL }) {
  ...
  const dotStyle = (i) => ({
    ...
    background: step === i ? ACCENT : BORDER, transition: 'all 0.3s',
  });
      ...
      <div style={{ width: '100%', maxWidth: 420, padding: '0 24px' }}>
        ...
        {step === 0 && (
          <div style={{ animation: 'fadeIn 0.3s ease' }}>
            ...
            <button onClick={() => setStep(1)} disabled={!weightOk} style={{
              width: '100%', padding: 16, borderRadius: 12, border: 'none',
              background: weightOk ? `linear-gradient(135deg,${ACCENT},#0a9e8e)` : CARD,
              color: weightOk ? '#fff' : MUTED, fontSize: 14, fontWeight: 700,
              fontFamily: MONO, cursor: weightOk ? 'pointer' : 'not-allowed',
            }}>{tl("continuer")}</button>
          </div>
        )}
        ...
        {step === 1 && (
          <div style={{ animation: 'fadeIn 0.3s ease' }}>
            ...
            <div style={cardStyle}>
              ...
              <div>
                <label style={labelStyle}>{tl("glycemieCible")}</label>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <input type="range" min={1.0} max={1.8} step={0.1} value={targetG} onChange={e => setTargetG(Number(e.target.value))} style={{ flex: 1, accentColor: '#22c55e' }} />
                  <div style={{ background: '#070c12', border: `1px solid rgba(34,197,94,0.3)`, borderRadius: 8, padding: '8px 14px', minWidth: 100, textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setStep(0)} style={{ flex: 1, padding: 14, borderRadius: 12, border: `1px solid ${BORDER}`, background: 'transparent', color: MUTED, fontSize: 13, fontFamily: MONO, cursor: 'pointer' }}>{tl("retour")}</button>
              <button onClick={() => setStep(2)} style={{ flex: 2, padding: 14, borderRadius: 12, border: 'none', background: `linear-gradient(135deg,${ACCENT},#0a9e8e)`, color: '#fff', fontSize: 14, fontWeight: 700, fontFamily: MONO, cursor: 'pointer' }}>{tl("continuer")}</button>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div style={{ animation: 'fadeIn 0.3s ease' }}>
            ...
            <div style={cardStyle}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                {[
                  ...
                  { label: tl("glycemieCible"), val: `${targetG.toFixed(1)} g/L`, color: '#22c55e', icon: "🎯" },
                ].map((it, i) => (
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setStep(1)} style={{ flex: 1, padding: 14, borderRadius: 12, border: `1px solid ${BORDER}`, background: 'transparent', color: MUTED, fontSize: 13, fontFamily: MONO, cursor: 'pointer' }}>{tl("retour")}</button>
              <button onClick={finish} style={{ flex: 2, padding: 16, borderRadius: 12, border: 'none', background: `linear-gradient(135deg,${ACCENT},#0a9e8e)`, color: '#fff', fontSize: 15, fontWeight: 700, fontFamily: MONO, cursor: 'pointer' }}>{tl("commencer")}</button>
            </div>
          </div>
        )}
      </div>
