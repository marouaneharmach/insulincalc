...
export default function QtyStepper({ food, mult, onChange, t }) {
  ...
  const qLabel = t ? t("quantite") : "QUANTITÉ";
  return (
    <div style={{ background: "#070c12", border: "1px solid #1c2a38", borderRadius: 12, padding: "12px 14px", marginTop: 8 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <div><span style={{ fontSize: 12, color: "#4a6070", letterSpacing: 1 }}>{qLabel} · </span><span style={{ fontSize: 12, color: "#7dd3fc", fontWeight: 700 }}>{ai >= 0 ? profile[ai].l : `×${mult}`}</span><span style={{ fontSize: 12, color: "#4a6070" }}> · {food.unit}</span></div>
        <div><span style={{ fontSize: 12, color: "#4a6070" }}>{t ? t("glucidesUnit") : "glucides"} : </span><span style={{ fontSize: 16, color: "#0ea5e9", fontWeight: 700 }}>{rc}g</span></div>
      </div>
      ...
      <div style={{ marginTop: 8 }}>
        ...
        <div style={{ fontSize: 12, color: "#2d3f50", marginTop: 4 }}>{food.unit} × {mult} = <strong style={{ color: "#3d7fa0" }}>{rc}g</strong> {t ? t("glucidesUnit") : "glucides"}{food.note ? <span style={{ color: "#1a3040" }}> · {food.note}</span> : ""}</div>
      </div>
    </div>
  );
}